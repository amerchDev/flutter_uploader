...
public class SwiftFlutterUploaderPlugin: NSObject, FlutterPlugin, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    private func enqueueMethodCall(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        let ebdString = args["earliestbegindate"] as? String
        var earliestBeginDate=Date()
        if let ebdString=ebdString{
            let dateFormatter = DateFormatter()
            dateFormatter.locale = Locale(identifier: "en_US_POSIX") // set locale to reliable US_POSIX
            dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            earliestBeginDate = dateFormatter.date(from:(ebdString as NSString).substring(to: 19))!
        }
        ...
        if (!validHttpMethods.contains(httpMethod)) {
            ...
            print("FLUTTER_UPLOADER-----------------------invalid_method");
            return
        }

        if files == nil || files!.count <= 0 {
            result(FlutterError(code: "invalid_files", message: "There are no items to upload", details: nil))
            print("FLUTTER_UPLOADER-----------------------invalid_files");
            return
        }

        guard let url = URL(string: urlString) else {
            result(FlutterError(code: "invalid_url", message: "url is not a valid url", details: nil))
            print("FLUTTER_UPLOADER-----------------------invalid_url");
            return
        }

        uploadTaskWithURLWithCompletion(url: url, files: files!, method: method, headers: headers, parameters: data, tag: tag, earliestBeginDate: earliestBeginDate, completion: { (task, error) in
            if (error != nil) {
                print("FLUTTER_UPLOADER-----------------------error");
                result(error!)
            } else {

                print("FLUTTER_UPLOADER-----------------------all good");
                let uploadTask = task!
    ...
    private func enqueueBinaryMethodCall(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        if (!validHttpMethods.contains(httpMethod)) {
            result(FlutterError(code: "invalid_method", message: "Method must be either POST | PUT | PATCH", details: nil))
                print("FLUTTER_UPLOADER----------------------binary-invalid_method");
            return
        }

        guard let url = URL(string: urlString) else {
            result(FlutterError(code: "invalid_url", message: "url is not a valid url", details: nil))
                print("FLUTTER_UPLOADER----------------------binary-invalid_url");
            return
        }

        guard let f = file else {
            result(FlutterError(code: "invalid_file", message: "file is not a valid path", details: nil))
                print("FLUTTER_UPLOADER----------------------binary-invalid_path");
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: info.path) else {
            result(FlutterError(code: "invalid_file", message: "file does not exist", details: nil))
                print("FLUTTER_UPLOADER----------------------binary-invalid_file");
            return
        }

        binaryUploadTaskWithURLWithCompletion(url: url, file: fileUrl, method: method, headers: headers, tag: tag, earliestBeginDate: earliestBeginDate, completion: { (task, error) in
            if (error != nil) {

                print("FLUTTER_UPLOADER----------------------binary-error");
                result(error!)
            } else {
                print("FLUTTER_UPLOADER----------------------binary-allgood");
                let uploadTask = task!
                let taskId = self.identifierForTask(uploadTask)
                self.runningTaskById[taskId] = UploadTask(taskId: taskId, status: .enqueue, progress: 0, tag: tag)
                result(taskId)
    ...
    private func uploadTaskWithURLWithCompletion(url: URL, files: [Any],
            ...
            if fileCount <= 0 {
                ...
                if let earliestBeginDate = earliestBeginDate {
                    ebd = earliestBeginDate
                }

                self.makeRequest(path, url, method, headers, formData.contentType, formData.contentLength, tout, ebd, completion: {
                    (task, error) in
    ...
    private func makeRequest(_ path: String, _ url: URL, _ method: String, _ headers: [String: Any?]?, _ contentType: String, _ contentLength: UInt64, _ timeout: Int, _ earliestBeginDate: Date, completion completionHandler: (URLSessionUploadTask?, FlutterError?) -> Void) {
        ...
        if #available(iOS 11, *) {
        uploadTask.earliestBeginDate = earliestBeginDate//Date().addingTimeInterval(5*60)
        }
        uploadTask.resume()
        completionHandler(uploadTask, nil)
    }
